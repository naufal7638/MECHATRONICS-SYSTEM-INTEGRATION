...
extern Servo myservo;   // <-- MODIFICATION: Access the servo object from .ino
static int current_servo_angle = 90; // <-- NEW: Global variable for servo position
...
// <-- NEW: Add a simple constrain function since we are in a .cpp file
static int a_constrain(int val, int min_val, int max_val) {
...
static esp_err_t capture_handler(httpd_req_t *req)
{
    ...
    {
        myservo.write(90); // <-- MODIFICATION: Reset servo to center
#endif
#if ARDUHAL_LOG_LEVEL >= ARDUHAL_LOG_LEVEL_INFO
        size_t fb_len = 0;
#endif
    ...
    if (fb->format == PIXFORMAT_RGB565
     ...
     && !recognition_enabled
        ...
        if (results.size() > 0) {
            ...
            // <-- MODIFICATION: Face Tracking Logic -->
            int face_x_center = (results.front().box[0] + results.front().box[2]) / 2; // <-- Used . instead of ->
            int frame_x_center = rfb.width / 2;
            int error = face_x_center - frame_x_center;
            
            // Adjust angle based on error (this is a simple P-controller)
            // You may need to tune the "0.1" (Kp value)
            current_servo_angle -= (int)(error * 0.1);
            current_servo_angle = a_constrain(current_servo_angle, 0, 180);
            myservo.write(current_servo_angle);
            // <-- END: Face Tracking Logic -->
...
#endif
#if CONFIG_ESP_FACE_RECOGNITION_ENABLED
            if (0) { // <-- MODIFICATION: Disabled face recognition
...
static esp_err_t cmd_handler(httpd_req_t *req)
{
    ...
    /* <-- MODIFICATION: Disabled face recognition & enroll UI
#if CONFIG_ESP_FACE_RECOGNITION_ENABLED
...
#endif
    */ // <-- MODIFICATION: End of disabled block
#endif
    else {
